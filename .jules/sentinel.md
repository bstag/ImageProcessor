...
## 2024-05-27 - Cross-Platform Path Traversal (Drive Letters & ADS)
**Vulnerability:** Filename sanitization relied solely on `os.path.basename` (after converting `\` to `/`). On Linux, `os.path.basename("C:passwd.jpg")` returns `"C:passwd.jpg"`. If a Windows user downloads this or extracts it from a ZIP, the OS interprets it as a drive-letter path (writing outside the intended directory) or an Alternate Data Stream (ADS). Furthermore, Windows forbids characters like `<`, `>`, `?`, `*`, `|`, and `"`, which can cause errors or allow security issues.
**Learning:** `os.path.basename` behavior is dependent on the host OS executing the Python code. Running it on Linux will not adequately sanitize Windows-specific path components or illegal characters.
**Prevention:** When sanitizing user-provided filenames for cross-platform safety, complement `os.path.basename` with an exclusion regex (e.g., `re.sub(r'[:*?"<>|]', '_', filename)`) to remove OS-specific illegal characters while preserving internationalization.
