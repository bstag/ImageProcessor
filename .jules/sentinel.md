...
## 2024-05-27 - Disk Exhaustion (DoS) Prevention in Logging
**Vulnerability:** The application used Python's standard `logging.FileHandler`, which append logs to a single file indefinitely. Under heavy use or attack, this can lead to unbounded log file growth, resulting in Disk Exhaustion (a form of Denial of Service) which can crash the application or the host system.
**Learning:** Standard file handlers do not manage disk space. Security applications and services exposed to untrusted input must control their resource footprint, including log files.
**Prevention:** Always use `logging.handlers.RotatingFileHandler` (or `TimedRotatingFileHandler`) with strict `maxBytes` and `backupCount` limits to ensure log files are automatically rotated and capped at a predictable maximum size.
